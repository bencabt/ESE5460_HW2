...
##added after training on colab 
MODEL_SAVE_PATH = "weights/allcnn_cifar10_20251022_195204.pth"

# Data transforms as defined
# Normalize for Cifar10 dataset given from online sources 
...
optimizer = torch.optim.SGD(model.parameters(), lr=.1, momentum=.9, weight_decay=1e-3) #lr, momentum, weight decay as defined
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 80], gamma=0.1) #take care of LR changes based on epoch
...
if os.path.exists(MODEL_SAVE_PATH):
    ...
    #Saving the model - had to look up how to do properly
...
#attempting to reverse normalize transformation
...
def plot_image_and_gradient(X, dx, title, index):
    ...
    X_np = X[index].detach().cpu().numpy().transpose(1, 2, 0) 
...
###PART 3(b) 2###
eps = 8.0 / 255.0  # given
mini_batch_step_losses = torch.zeros(5).to(device) #storage
total_images_processed = 0 #iter
...
for x,y in zip(X, y):
    ...
    for k in range(5):
        if x.grad is not None:
            x.grad.zero_()
        # forward propagate x through the network, backprop loss
        output = model(x)
        loss = loss_fn(output, y)
        loss.backward()
